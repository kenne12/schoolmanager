...
@ManagedBean
@ViewScoped
public class ProcesVerbalNoteAnnuelCtrl extends AbstractProcesVeralNoteAnnuelCtrl {

    /**
     * Creates a new instance of ProcesVerbalNoteSequentielCtrl
     */
    public ProcesVerbalNoteAnnuelCtrl() {
    }
    ...
    public void imprimerProces() {
        try {
            ...
            fichier = "proces_verbal_note" + "_" + annee.getCode() + "-" + annee.getCodefin() + "_" + categorie.getIdclasse().getNom() + ".pdf";
            PrintUtils.printAnnualReportNote(annee, trimesteannee, categorie.getIdclasse(), classeElementevaluation, eleveanneeclasses, trimesteannees, sequenceannees, classeElementevaluationFacadeLocal, planningEvaluationFacadeLocal, evaluationFacadeLocal,sequenceanneeFacadeLocal);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public void loadSequence() {
        sequenceannees.clear();
        try {
            for (Trimesteannee t : trimesteannees) {
                List<Sequenceannee> r = sequenceanneeFacadeLocal.getByTrimestre(t.getIdtrimestrean());
                if (!r.isEmpty()) {
                    sequenceannees.addAll(r);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Integer sequenceSize(Trimesteannee t) {
        return sequenceanneeFacadeLocal.getByTrimestre(t.getIdtrimestrean()).size();
    }

    public List<Sequenceannee> findByTrimestre(Trimesteannee trimesteannee) {
        return sequenceanneeFacadeLocal.getByTrimestre(trimesteannee.getIdtrimestrean());
    }
